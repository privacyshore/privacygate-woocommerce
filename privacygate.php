<?php
/*
Plugin Name:  PrivacyGate
Plugin URI:   https://github.com/privacyshore/privacygate-woocommerce/
Description:  A payment gateway that allows your customers to pay with cryptocurrency via privacygate Commerce (https://dash.privacygate.io/)
Version:      1.1.4
Author:       PrivacyGate
Author URI:   https://dash.privacygate.io/
License:      GPLv3+
License URI:  https://www.gnu.org/licenses/gpl-3.0.html
Text Domain:  privacygate
Domain Path:  /languages

WC requires at least: 3.0.9
WC tested up to: 3.6.3

privacygate Commerce is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
any later version.

privacygate Commerce is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with privacygate WooCommerce. If not, see https://www.gnu.org/licenses/gpl-3.0.html.
*/
...
function cb_init_gateway() {
	...
	if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		require_once 'class-wc-gateway-privacygate.php';
		...
		add_filter( 'woocommerce_payment_gateways', 'cb_wc_add_privacygate_class' );
...
function cb_wc_add_privacygate_class( $methods ) {
	$methods[] = 'WC_Gateway_privacygate';
	return $methods;
}

function cb_wc_check_orders() {
	$gateway = WC()->payment_gateways()->payment_gateways()['privacygate'];
...
function cb_wc_register_blockchain_status() {
	register_post_status( 'wc-blockchainpending', array(
		'label'                     => __( 'Blockchain Pending', 'privacygate' ),
		'public'                    => true,
...
function cb_wc_add_status( $wc_statuses_arr ) {
	...
	foreach ( $wc_statuses_arr as $id => $label ) {
		...
		if ( 'wc-pending' === $id ) {  // after "Payment Pending" status.
			$new_statuses_arr['wc-blockchainpending'] = __( 'Blockchain Pending', 'privacygate' );
		}
	}
...
/**
 * Add order privacygate meta after General and before Billing
 *
...
{
    if ($order->get_payment_method() == 'privacygate') {
        ?>

        <br class="clear"/>
        <h3>PrivacyGate Data</h3>
        <div class="">
            <p>PrivacyGate Reference # <?php echo esc_html($order->get_meta('_privacygate_charge_id')); ?></p>
        </div>
...
/**
 * Add privacygate meta to WC emails
 *
...
function cb_custom_woocommerce_email_order_meta_fields( $fields, $sent_to_admin, $order ) {
    if ($order->get_payment_method() == 'privacygate') {
        $fields['privacygate_commerce_reference'] = array(
            'label' => __( 'privacygate Commerce Reference #' ),
            'value' => $order->get_meta( '_privacygate_charge_id' ),
        );
    }
