...
/**
 * Sends API requests to privacygate.
 */
class privacygate_API_Handler {
	...
	/** @var string privacygate API url. */
	public static $api_url = 'https://api.privacygate.io/';

	/** @var string privacygate API version. */
	public static $api_version = '2018-03-22';

	/** @var string privacygate API key. */
	...
	public static function send_request( $endpoint, $params = array(), $method = 'GET' ) {
		// phpcs:ignore
		self::log( 'privacygate Request Args for ' . $endpoint . ': ' . print_r( $params, true ) );
		$args = array(
			'method'  => $method,
			'headers' => array(
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( in_array( $code, array( 200, 201 ), true ) ) {
				...
				$errors = array(
					...
					429 => 'privacygate API rate limit exceeded.',
				);
