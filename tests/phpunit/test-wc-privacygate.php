...
class WC_privacygate_Test extends WP_UnitTestCase {
	/**
	 * Test that privacygate payment gateway has loaded.
	 */
	public function test_gateway_loaded() {
		$this->assertArrayHasKey( 'privacygate', WC()->payment_gateways()->payment_gateways() );
	}
	...
	public function test_multiple_icons() {
		$payment_gateway = WC()->payment_gateways->payment_gateways()['privacygate'];

		// Test 2 icons are loaded for bitcoin and litecoin.
		update_option( 'privacygate_payment_methods', array( 'bitcoin', 'litecoin' ) );
		...
		// Cleanup
		delete_option( 'privacygate_payment_methods' );
	}
	...
	public function test_ignore_icons() {
		...
		update_option( 'privacygate_payment_methods', array( 'bitcoin', 'notacoin' ) );
		$icons   = $payment_gateway->get_icon();
		$matches = array();
		$num     = preg_match_all( '/<img .*? src="(.*?)".*?\/>/', $icons, $matches );
	...
	public function test_order_checkout() {
		...
		// Set payment method to privacygate.
		$payment_gateway = WC()->payment_gateways->payment_gateways()['privacygate'];
